      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-[400px]">
        <div className="flex justify-between items-center">
          <h2 className="text-xl text-[#2899CB] font-bold">
            {mode === 'edit' ? 'EDIT DETAILS' : mode === 'delete' ? 'DELETE' : 'VIEW DETAILS'}
          </h2>
            ...
            {viewMode === 'profile' ? (
              ...
              <>
                <div className="flex">
                  ...
                  <p className="ml-14">{user.dname}</p>
                </div>
                <div className="flex">
                  <p><strong className='text-sm'>Device Unique Number</strong></p>
                  <p className="ml-18 text-sm">{user.dnum}</p> {/* Apply text-sm class */}
                </div>
                <div className="flex">
                  <p><strong>MAC ID</strong></p>
                  <p className="ml-18">{user.macid}</p>
                </div>
              </>
            )}
          ...
          <form onSubmit={handleSubmit} className="space-y-2 mt-5 gap-3">
            {viewMode === 'profile' ? (
              ...
              <>
                ...
                <div className="flex flex-col">
                  <label><strong>Device Unique Number</strong></label>
                  <input
                    type="number"
                    name="dnum"
                    value={formData.dnum}
                    onChange={handleChange}
                    className="p-2 border border-gray-300 rounded text-sm" // Apply text-sm class
                  />
                </div>
                <div className="flex flex-col">
                  <label><strong>MAC ID</strong></label>
                  <input
                    type="text"
                    name="macid"
